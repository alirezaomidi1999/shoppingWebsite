...
function Basket() {
    ...
    const changeQuantityHandler = (basketproduct,value) => {
      ...
      const findProduct = basket.filter(productbasket => basketproduct.id === productbasket.id)
      findProduct[0].quantity = value
      setProductBasket(basket)
      setTotalPrice(basket.map(productbasket => productbasket.quantity * productbasket.price).reduce((a, b) => a + b, 0))
    }

    const removeProductFromBasket = (basketproduct) => {

      const basket = [...productBasket]
      const removeProduct = basket.filter(productbasket => basketproduct.id !== productbasket.id)
      const removeOfTotalPrice = basket.filter(productbasket => basketproduct.id === productbasket.id)
      console.log(basket)
      setProductBasket(removeProduct)
      setTotalPrice(totalPrice - removeOfTotalPrice[0].price * removeOfTotalPrice[0].quantity)
  ...
  return (
    ...
    showBasket && <div className='container-basket'>
       <GrClose onClick={closeBasket} style={{position:'absolute',right:10,top:5,cursor:'pointer'}}/>
       <div className="container-basketproduct">
         ...
         <div key={product.id} className='basketproduct'>
           ...
           <img className='basketproduct-image' src = {product.images[1]} alt = {product.images[2]}></img>
           <span className='basketproduct-title'>{product.title}</span>
           </div>
           <div className="basketproduct-right">
           <span className='basketproduct-price'>price: {product.price}$</span>
           <input className='basketproduct-quantity' min={1} max={50} value = {product.quantity} type='number' onChange={(e) => changeQuantityHandler(product, e.target.value)}></input>
           </div>
           <IoCloseCircleOutline className='icondelete' onClick={() => removeProductFromBasket(product)}></IoCloseCircleOutline>
