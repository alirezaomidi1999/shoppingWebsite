...
if(getLocalStorage !== null){
      setProductBasket(getLocalStorage.productBasket);
      setTotalPrice(getLocalStorage.totalPrice);
      setCounterProduct(getLocalStorage.counterProduct);
  ...
  useEffect(() => {
   if(flag){
     const addToLocalStorage = localStorage.setItem(
        "products",
        JSON.stringify({productBasket,totalPrice,counterProduct})
  ...
  }, [productBasket, totalPrice, counterProduct]);
  ...
  const addToBasket = (product) => {
    ...
    const updateBasket = newBasket.filter(
      (basketproduct) => basketproduct.id === product.id
    );
    const checkBasket = productBasket.some(
      (basketproduct) => basketproduct.id === product.id
    );
    const indexBasket = productBasket.findIndex(
      (basketproduct) => basketproduct.id === product.id
    );
    ...
    if (checkBasket) {
      productBasket[indexBasket].quantity =
        productBasket[indexBasket].quantity + 1;
    ...
    // const addToLocalStorage = localStorage.setItem(
    //     "products",
    //     JSON.stringify({"productBasket": [...productBasket, ...updateBasket], "totalPrice": totalPrice,"counterprice":counterProduct})
